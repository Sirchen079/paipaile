...
import { makeAuth, makeLoginThrottle, parseCookies } from './auth';
...
const auth = makeAuth(COOKIE_SECRET, ACCESS_PASSWORD);
...
app.post('/api/login', (req, res) => {
  ...
  if (typeof password === 'string' && auth.checkPassword(password)) {
    throttle.reset(ip);
    const { token, maxAge } = auth.issueToken();
...
app.get('/api/me', (req, res) => {
  ...
  res.status(auth.verifyToken(token) ? 200 : 401).json({ ok: auth.verifyToken(token) });
});
